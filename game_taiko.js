...
function judgeHold(note, api){
  ...
  if(result==="miss"){
    S.combo=0; 
    S.gauge=Math.max(0,S.gauge+GA_MISS);
...
function judgeBalloon(note, api){
  ...
  if(note.fill>=1.0){ 
    result="great"; 
    gaugeDelta=GA_BALLOON_PASS; 
    scoreBase=SC_GREAT*2; 
  }
  else if(note.fill>=0.6){ 
    result="good"; 
    gaugeDelta=GA_BALLOON_HALF; 
    scoreBase=SC_GOOD*1.5; 
  }
  else { 
    result="miss"; 
    gaugeDelta=GA_BALLOON_FAIL; 
    scoreBase=0; 
  }
  note.judged=result;
  ...
  if(result==="miss"){ 
    S.combo=0; 
    popJudge("氣球沒吹滿", api.colors.dim, api);
    if (S.sound) S.sound.play('taiko_miss');
  } else { 
    S.combo++; 
    S.bestCombo=Math.max(S.bestCombo,S.combo); 
    S.score+=Math.round(scoreBase);
    popJudge(result==="great"?"氣球吹滿！":"氣球有到一半！", 
             result==="great"?api.colors.gold:api.colors.green, api);
    if (S.sound) {
      if (result === 'great') S.sound.play('taiko_great');
      else S.sound.play('taiko_good');
    }
...
function render(g,w,h,api){
  const C=g.colors;
  g.ctx.fillStyle=C.header; 
  g.ctx.fillRect(0,0,w,50);
  ...
  g.rrect(0,top,w,bot,0); 
  g.fill(C.track);
  g.ctx.globalAlpha=0.4; 
  g.rrect(0,top,judgeX,bot,0); 
  g.fill(C.panel); 
  g.ctx.globalAlpha=1;
  g.line(0,top,w,top,"#39405f",1);
  g.line(0,bot,w,bot,"#39405f",1);
  if(S.flash>0.05){ 
    g.ctx.globalAlpha=S.flash*0.5; 
    g.rrect(judgeX-6,top,judgeX+6,bot,0); 
    g.fill(C.gold); 
    g.ctx.globalAlpha=1; 
  }
  ...
  g.rrect(gx0,gy-9,gx1,gy+9,6); 
  g.fill(C.track);
  const clearX=gx0+(gx1-gx0)*GAUGE_CLEAR/100;
  const gw=(gx1-gx0)*S.gauge/100;
  g.rrect(gx0,gy-9,gx0+Math.max(2,gw),gy+9,6); 
  g.fill(S.gauge>=GAUGE_CLEAR?C.gold:C.redBr);
  ...
  for(const n of CHART.notes){
    ...
    if(n.type==="hold"){
      ...
      g.rrect(x0,laneY-24,x1,laneY+24,14); 
      g.fill(col);
      if(n.holdHit>0){
        const fx = x0 + (n.holdHit/n.dur)*(x1-x0);
        g.rrect(x0,laneY-24,fx,laneY+24,14); 
        g.fill(C.gold);
      }
      g.stroke(C.goldDk,2);
  ...
  g.rrect(mx0,my,mx1,my+22,8); 
  g.fill(C.track);
  const frac=Math.min(1,(S.liveFlow||0)/150);
  if(frac>0.02){ 
    g.rrect(mx0,my,mx0+(mx1-mx0)*frac,my+22,8); 
    g.fill(C.gold); 
  }
