...
function reset(api){
  S = { 
    ...
    flew:false, 
    flyY:0, 
    success:api.store.get("balloon_best",0),
    inBand:0, 
    fb:"深吸一口氣，穩穩地把氣吐出去～", 
    fbCol:api.colors.cream,
    steadyBonus:0, 
    wrong:false, 
    sparkle:[], 
    hue:0, 
...
function update(dt, input, api){
  ...
  if(S.flew){ 
    S.flyY += dt*0.6; 
    for(const s of S.sparkle){ s.life-=dt; s.y-=dt*40; }
    S.sparkle=S.sparkle.filter(s=>s.life>0);
    if(S.flyY>1.4){ /* 等使用者按換一顆 */ } 
    return; 
  }

  const blowing = flow>=P.LO;
  if(blowing){
    // 力道越大脹越快:在 LO~HI 之間,線性內插 FILL_BASE ~ FILL_MAX
    ...
    if (S.sound && Math.floor(S.size * 10) > Math.floor(oldSize * 10)) {
      if (S.size >= 0.9) {
        S.sound.play('balloon_90');  // 90% 緊張提示
      } else {
        S.sound.play('balloon_pop'); // 一般進度
      }
    }
  }
  else { 
  ...
  if(S.size>=0.999){
    ...
    // 🎵 音效：放飛 + 里程碑 + 破紀錄
    if (S.sound) {
      S.sound.play('balloon_fly');     // 放飛音效
      if (S.success === 1 || S.success % 5 === 0) {
        S.sound.play('level_up');      // 每5顆里程碑
      }
      if (S.success > best) {
        S.sound.play('record_break');  // 破紀錄
      }
    }
    ...
    for(let i=0;i<28;i++){ 
      S.sparkle.push({
        ...
        col:[api.colors.gold,api.colors.redBr,api.colors.green,api.colors.blue][i%4],
        sz:3+Math.random()*4
      }); 
    }
  }
...
function render(g,w,h,api){
  ...
  const hue=S.hue;
  grd.addColorStop(0,`hsl(${hue},90%,75%)`); 
  grd.addColorStop(1,`hsl(${hue},80%,55%)`);
  g.ctx.fillStyle=grd; 
  g.ctx.beginPath(); 
  g.ctx.ellipse(cx,by,R*0.92,R,0,0,6.283); 
  g.ctx.fill();
  g.ctx.fillStyle=`hsl(${hue},80%,55%)`; 
  g.ctx.beginPath(); 
  g.ctx.moveTo(cx-6,by+R-2); 
  g.ctx.lineTo(cx+6,by+R-2); 
  g.ctx.lineTo(cx,by+R+10); 
  g.ctx.fill();
  // 高光
  g.ctx.globalAlpha=0.5; 
  g.circle(cx-R*0.32,by-R*0.34,R*0.16,"#ffffff"); 
  g.ctx.globalAlpha=1;
  ...
  for(const s of S.sparkle){ 
    g.ctx.globalAlpha=Math.max(0,s.life); 
    g.circle(cx+s.x*140,by+s.y*160,s.sz,s.col); 
    g.ctx.globalAlpha=1; 
  }
